...
from src.application.schemas.cards import CardSchema, CreateCardDBSchema
    ...
    async def __call__(self, cards: None = None) -> list[CardSchema]:
        ...
        async with self.session.begin():
            ...
            for info_item in data:
                information = await self.create_information(
                    CreateInformationSchema(title=info_item["information_title"])
                )
